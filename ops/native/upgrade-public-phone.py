#!/usr/bin/env python3
"""Fixed native-4 to native-5 upgrade. Python 3.6; secret values never logged."""
import argparse
import hashlib
import json
import os
...
OPS = Path('/root/duduhire-upgrade-native5')
OLD = Path('/opt/duduhire/releases/duduhire-20260913-native-4')
NEW = Path('/opt/duduhire/releases/duduhire-20260913-native-5')
...
DELTA = Path('/root/duduhire-native-5-delta.tar.gz')
DELTA_SHA = '1030a66e0b31bcc92d090a165303178cf357df105d42b90405997f9d9efd9f36'
MANIFEST_SHA = 'e9d8bacf0fd33521e56fe32805064d63be1f04681ca8146c8ccee61a0453c147'
...
def replace_file(path, data, mode):
    tmp = path.parent / ('.' + path.name + '.native5-next')
...
def point_current(target):
    tmp = CURRENT.parent / '.current.native5-next'
...
def negative_auth(port, values):
    ...
    email='native5-acceptance@example.invalid'
...
def stage():
    ...
    print('NATIVE5_STAGED; production_unchanged=true',flush=True)
...
def activate():
    ...
    try:
        ...
        replace_file(UNIT,(OPS/'unit.before').read_bytes().replace(str(OLD).encode(),str(NEW).encode())
                     .replace(b'duduhire-20260913-native-4',b'duduhire-20260913-native-5'),0o644)
    ...
    write_new(OPS/'COMPLETE',b'native5 activated; public browser acceptance remains\n')
    print('NATIVE5_ACTIVATED; browser_and_stability_acceptance_pending=true',flush=True)
