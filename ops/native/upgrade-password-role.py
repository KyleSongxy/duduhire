#!/usr/bin/env python3
"""Fixed native-5 to native-6 upgrade with additive auth migrations. Python 3.6; secret values never logged."""
import argparse
import hashlib
import json
import os
...
OLD = Path('/opt/duduhire/releases/duduhire-20260913-native-5')
NEW = Path('/opt/duduhire/releases/duduhire-20260916-native-6')
...
DELTA = Path('/root/duduhire-native-6-delta.tar.gz')
DELTA_SHA = 'f56d023f305cc3d72d09d5abb14bbf42af23f6b7094a91e3ea38e08e80c275bc'
MANIFEST_SHA = '8d864431ffa44a1eed5053b3231cebbc95cc5d483e4c7cce9e2f69736bfeba3f'
...
def replace_file(path, data, mode):
    tmp = path.parent / ('.' + path.name + '.native6-next')
...
def point_current(target):
    tmp = CURRENT.parent / '.current.native6-next'
...
def verify_candidate():
    ...
    expected['PHONE_AUTH_ALLOW_ALL_NUMBERS']='true'
    expected['PHONE_AUTH_DAILY_LIMIT']='30'
    expected.pop('PHONE_AUTH_SEND_UNTIL',None)
...
def negative_auth(port, values):
    ...
    email='native6-acceptance@example.invalid'
...
def password_checks(port):
    status,body=request(port,'/api/v1/auth/password/login',{'email':'native6-acceptance@example.invalid','password':'synthetic-not-a-real-account-password'})
    require(status==401 and body['error']['code']=='INVALID_CREDENTIALS','password_login_regression')
    status,body=request(port,'/api/v1/auth/password/setup')
    require(status==401 and body['error']['code']=='AUTH_REQUIRED','password_setup_access_regression')
    status,body=request(port,'/api/v1/auth/role',{'role':'talent'})
    require(status==401,'role_unauthorized_regression')
    print('password_negative_and_role_authorization_checks=true; real_accounts_modified=false',flush=True)

def migrate_and_grant():
    from urllib.parse import urlparse,unquote
    phase('additive_migrations_and_runtime_grants')
    migration=parse_env(Path('/etc/duduhire/migration.env'))
    runtime=parse_env(RUNTIME)
    maintenance=parse_env(Path('/etc/duduhire/maintenance.env'))
    env=dict(migration);env.update(ENV)
    require(env.get('MIGRATION_DATABASE_SSL')=='true','migration_tls_required')
    url=urlparse(env['MIGRATION_DATABASE_URL'])
    runtime_role=unquote(urlparse(runtime['DATABASE_URL']).username)
    maintenance_url=maintenance.get('MAINTENANCE_DATABASE_URL')
    require(maintenance_url is not None,'maintenance_url_missing')
    maintenance_role=unquote(urlparse(maintenance_url).username)
    database=unquote(url.path.lstrip('/'))
    require(database=='duduhire' and unquote(url.username)=='duduhire_owner','unexpected_migration_identity')
    pg_env=dict(ENV)
    pg_env.update(PGHOST=url.hostname,PGPORT=str(url.port or 5432),PGUSER=unquote(url.username),PGPASSWORD=unquote(url.password),PGDATABASE=database,PGSSLMODE='verify-full',PGSSLROOTCERT='/etc/duduhire/ca.crt')
    psql=shutil.which('psql',path=ENV['PATH'])
    require(psql is not None,'psql_missing')
    def sql(query):
        return run([psql,'-X','-A','-t','-v','ON_ERROR_STOP=1','-c',query],env=pg_env).decode().strip()
    counts="SELECT json_build_object('users',(SELECT count(*) FROM users),'examples',(SELECT count(*) FROM matching_examples),'listings',(SELECT count(*) FROM matching_listings),'sessions',(SELECT count(*) FROM sessions))"
    before=sql(counts)
    write_new(OPS/'database-counts.before.json',before.encode())
    output=run(['/usr/local/bin/node',str(NEW/'apps/api/dist/migrate.js')],env=env,timeout=120)
    write_new(OPS/'migration-output.txt',output)
    print(output.decode(),flush=True)
    grants=run([psql,'-X','-v','ON_ERROR_STOP=1','-v','database_name='+database,'-v','runtime_role='+runtime_role,'-v','maintenance_role='+maintenance_role,'-f',str(NEW/'apps/api/sql/grant-database-roles.sql')],env=pg_env)
    write_new(OPS/'grant-output.txt',grants)
    history=json.loads(sql("SELECT json_object_agg(name,checksum) FROM schema_migrations"))
    expected={Path(k).name:v for k,v in manifest((NEW/'SHA256SUMS').read_bytes()).items() if k.startswith('apps/api/migrations/')}
    require(history==expected,'database_migration_history_mismatch')
    after=sql(counts)
    write_new(OPS/'database-counts.after.json',after.encode())
    b=json.loads(before);a=json.loads(after)
    require(all(a[k]>=b[k] for k in b),'database_rows_lost')
    require(sql("SELECT count(*) FROM sessions WHERE active_role IS NULL OR active_role NOT IN ('client','talent')")=='0','session_role_backfill_invalid')
    print('migration_history=11; existing_counts_before='+before+'; after='+after,flush=True)
...
def stage():
    ...
    require(original.get('PHONE_AUTH_ENABLED')=='true' and original.get('PHONE_AUTH_ALLOW_ALL_NUMBERS')=='true' and original.get('PHONE_AUTH_DAILY_LIMIT')=='30' and 'PHONE_AUTH_SEND_UNTIL' not in original,'public_sms_policy_changed')
    ...
    for name in ['package-lock.json','apps/api/package.json','apps/web/package.json']:
        require(digest(OLD/name)==digest(NEW/name),'dependency_manifest_changed')
    old_root=json.loads((OLD/'package.json').read_text());new_root=json.loads((NEW/'package.json').read_text())
    old_root.pop('scripts',None);new_root.pop('scripts',None)
    require(old_root==new_root,'root_dependency_metadata_changed')
    old_migrations={k:v for k,v in old_manifest.items() if k.startswith('apps/api/migrations/')}
    new_migrations={k:v for k,v in desired.items() if k.startswith('apps/api/migrations/')}
    require(len(old_migrations)==9 and len(new_migrations)==11,'unexpected_migration_inventory')
    require(all(new_migrations.get(k)==v for k,v in old_migrations.items()),'historical_migration_changed')
    require(set(new_migrations)-set(old_migrations)=={'apps/api/migrations/010_password_authentication.sql','apps/api/migrations/011_session_active_role.sql'},'unexpected_new_migration')
    ...
    candidate['PHONE_AUTH_ALLOW_ALL_NUMBERS']='true'
    candidate['PHONE_AUTH_DAILY_LIMIT']='30'
    candidate.pop('PHONE_AUTH_SEND_UNTIL',None)
    write_new(OPS/'runtime.candidate.env', ''.join(k+"='"+v+"'\n" for k,v in candidate.items()).encode())
    verify_candidate()
    phase('public_phone_configuration')
    code="import {loadPhoneAuthConfig} from '"+str(NEW/"apps/api/dist/phoneAuthConfig.js")+"';const c=loadPhoneAuthConfig(process.env);if(!c.enabled||!c.allowAllNumbers||c.dailyLimit!==30||c.sendUntil!==undefined)throw Error('invalid');console.log('public_mainland=true; daily_limit=30; deadline_removed=true; sms_sent=0');"
    check_env=dict(candidate);check_env.update(ENV)
    print(run(['/usr/local/bin/node','--input-type=module','-e',code],env=check_env).decode(),flush=True)
    migrate_and_grant()
    ...
    write_new(OPS/'STAGED',b'payload, public phone configuration, preview health and negative auth passed\n')
    print('NATIVE6_STAGED; production_app_unchanged=true; additive_migrations_applied=true',flush=True)
...
def activate():
    ...
    try:
        ...
        replace_file(UNIT,(OPS/'unit.before').read_bytes().replace(str(OLD).encode(),str(NEW).encode())
                     .replace(b'duduhire-20260913-native-5',b'duduhire-20260916-native-6'),0o644)
        run(['systemctl','daemon-reload'])
        run(['systemctl','restart',UNIT.name])
        ready(8788)
        pid=run(['systemctl','show',UNIT.name,'--property=MainPID','--value']).decode().strip()
        running=dict(x.split(b'=',1) for x in Path('/proc/'+pid+'/environ').read_bytes().split(b'\0') if b'=' in x)
        require(running.get(b'PHONE_AUTH_ALLOW_ALL_NUMBERS')==b'true' and running.get(b'PHONE_AUTH_DAILY_LIMIT')==b'30' and b'PHONE_AUTH_SEND_UNTIL' not in running,'running_phone_config_mismatch')
    ...
    write_new(OPS/'COMPLETE',b'native6 activated; public browser acceptance remains\n')
    print('NATIVE6_ACTIVATED; browser_and_stability_acceptance_pending=true',flush=True)
