#!/usr/bin/env bash
# One attempt after database-access repair. Earlier drivers/attempts stay untouched.
set -euo pipefail
umask 077

usage() {
  cat <<'HELP'
Usage: bash stage-after-database-repair.sh --check | --run
--check is read-only: verify completed database repair, all three TLS identities,
  backup/release, and absent release/unit/current. No credentials are printed.
--run repeats those checks, then creates one private attempt directory and runs:
  extract -> dependencies -> validate -> migrate -> unit -> start
  -> current symlink -> cutover prepare. Public Nginx is never changed/reloaded.
This script never creates database roles/env, repeats repairs, or resumes itself.
...
finish() {
  code=$?
  if [ "$code" -ne 0 ]; then
    printf 'POST_DATABASE_FAILED phase=%s exit=%s; inspect partial state, do not rerun drivers\n' "$phase" "$code" >&2
...
attempt_path=$ops/stage-after-database-repair-attempt
...
for program in node python3 bash sha256sum tar ss systemctl tee psql; do
...
try:
    ...
    for name in ("backup-complete.json", "backup-manifest.json", "tls-complete.json", "database-env-complete.json"):
    ...
    check("zjad" in inventory["databases"] and "duduhire" in inventory["databases"]
          and set(inventory["roles"]) == set(bootstrap.ROLES), "completed_database_and_three_roles_required")
    database = json.loads((target / "database-env-complete.json").read_text())
    check(database["database"] == "duduhire" and set(database["roles"]) == set(bootstrap.ROLES)
          and database["apiPort"] == 8788, "database_completion_marker_invalid")
    for name in ("runtime", "migration", "maintenance"):
        regular(Path("/etc/duduhire") / (name + ".env"), 0o600)
    hba = Path(inventory["settings"]["hba_file"])
    check(not hba.is_symlink() and b"BEGIN DUDUHIRE" in hba.read_bytes(), "repaired_DuduHire_HBA_required")
    check(bootstrap.query("SELECT count(*) FROM pg_hba_file_rules WHERE error IS NOT NULL;") == "0",
          "HBA_parse_errors")
    print("preconditions_verified=true; backup_manifest=true; live_verified_TLS=true; database_marker=true")
except Exception as error:
    # Bootstrap's Stop/our own checks contain only fixed metadata; never dump input.
    detail = str(error) if isinstance(error, RuntimeError) else type(error).__name__
    print("ERROR post_database_preconditions=" + detail, file=sys.stderr)
    sys.exit(1)
PY

# Node parseEnv reads existing secrets; psql receives them only in its private env.
# No SQL mutation and no raw database/credential error output in this preflight.
node --input-type=module <<'NODE'
import {readFileSync} from 'node:fs';
import {parseEnv} from 'node:util';
import {spawnSync} from 'node:child_process';
const check = ok => { if (!ok) throw new Error(); };
try {
  const ca='/etc/duduhire/ca.crt';
  const profiles=[
    ['migration','MIGRATION_DATABASE','duduhire_owner'],
    ['runtime','DATABASE','duduhire_runtime'],
    ['maintenance','MAINTENANCE_DATABASE','duduhire_maintenance'],
  ];
  for(const [profile,prefix,role] of profiles) {
    const config=parseEnv(readFileSync('/etc/duduhire/'+profile+'.env','utf8'));
    check(config.NODE_ENV==='production' && config[prefix+'_SSL']==='true'
      && config.NODE_EXTRA_CA_CERTS===ca && !config.NODE_OPTIONS && !config.NODE_TLS_REJECT_UNAUTHORIZED);
    const url=new URL(config[prefix+'_URL']);
    check(['postgres:','postgresql:'].includes(url.protocol) && url.hostname==='localhost'
      && (!url.port || url.port==='5432') && decodeURIComponent(url.pathname)==='/duduhire'
      && decodeURIComponent(url.username)===role && url.password && !url.search && !url.hash);
    const env={PATH:process.env.PATH, PGHOST:'localhost', PGHOSTADDR:'127.0.0.1',
      PGPORT:'5432', PGDATABASE:'duduhire', PGUSER:role, PGPASSWORD:decodeURIComponent(url.password),
      PGSSLMODE:'verify-full', PGSSLROOTCERT:ca, PGCONNECT_TIMEOUT:'5', PGPASSFILE:'/dev/null',
      PGSERVICEFILE:'/dev/null', PGOPTIONS:'-c statement_timeout=5000 -c default_transaction_read_only=on'};
    const result=spawnSync('psql',['-X','-w','-qAt','-v','ON_ERROR_STOP=1','-c',
      "SELECT json_build_object('db',current_database(),'role',current_user,'tls',"+
      "(SELECT ssl FROM pg_stat_ssl WHERE pid=pg_backend_pid()),'elevated',"+
      "rolsuper OR rolcreatedb OR rolcreaterole OR rolreplication OR rolbypassrls,"+
      "'dbOwner',(SELECT pg_get_userbyid(datdba)=current_user FROM pg_database WHERE datname=current_database()),"+
      "'schemaOwner',(SELECT pg_get_userbyid(nspowner)=current_user FROM pg_namespace WHERE nspname='public'),"+
      "'ownerMember',pg_has_role(current_user,'duduhire_owner','MEMBER')) "+
      "FROM pg_roles WHERE rolname=current_user;"],
      {env,encoding:'utf8',stdio:['ignore','pipe','ignore'],timeout:10000});
    check(result.status===0);
    const row=JSON.parse(result.stdout.trim());
    check(row.db==='duduhire' && row.role===role && row.tls===true && row.elevated===false);
    check(role==='duduhire_owner' ? row.dbOwner && row.schemaOwner : !row.dbOwner && !row.schemaOwner && !row.ownerMember);
    const plaintext=spawnSync('psql',['-X','-w','-qAt','-c','SELECT 1;'],
      {env:{...env,PGSSLMODE:'disable'},stdio:'ignore',timeout:10000});
    check(plaintext.status!==0 && plaintext.status!==null && !plaintext.error);
  }
  console.log('database_preflight=true; three_TLS_identities=true; plaintext_rejected=true');
} catch {
  console.error('ERROR existing database/env/TLS identity validation failed; raw errors suppressed');
  process.exitCode=1;
}
NODE
...
if [ "$mode" = --check ]; then
  printf 'POST_DATABASE_CHECK_PASSED; read-only; repaired database/TLS/backup/release verified; release/unit/current absent\n'
  exit 0
fi
...
run_phase() {
  ...
  printf 'POST_DATABASE_PHASE_START %s\n' "$phase"
  "$@" 2>&1 | tee "$attempt/$phase.log"
  printf 'phase=%s time=%s\n' "$phase" "$(date -u +%FT%TZ)" > "$attempt/$phase.complete"
  printf 'POST_DATABASE_PHASE_COMPLETE %s\n' "$phase"
}
extract_release() {
  absent "$release"
