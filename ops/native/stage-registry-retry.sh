...
if [ "$step" = dependencies ]; then
  registry=${DUDUHIRE_NPM_REGISTRY:-}
  case "$registry" in
    https://registry.npmjs.org|https://registry.npmmirror.com) ;;
    *) fail 'Explicit DUDUHIRE_NPM_REGISTRY must be the approved npmjs or npmmirror HTTPS URL' ;;
  esac
  ...
  if ! runuser -u "$app_user" -- env -i \
    ...
    npm_config_cache="$release/.native-install/cache" npm_config_logs_max=5 \
    npm_config_registry="$registry" npm_config_replace_registry_host=npmjs \
    npm_config_fetch_timeout=60000 npm_config_fetch_retries=1 \
